...
localDebugger.on('connection', (socket) => {
  ...
  socket.on('data', (data) => {
    ...
    skillInvoker[lambdaHandlerName](body, null, (_invokeErr, response) => {
      ...
      socket.write(`HTTP/1.1 200 OK${httpHeaderDelimeter}Content-Type: application/json;charset=UTF-8${httpHeaderDelimeter}Content-Length: ${response.length}${httpBodyDelimeter}${jsonResponse}`);
    });
  });
});
...
* @param {defaultValue} defaultValue default value of the argument that is returned if the
* value doesn't exist
*/
